...
class PriceInDollorViewController: UIViewController ,UIPickerViewDelegate, UIPickerViewDataSource{
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int)
    {
        ...
        if let rate = obj.objResponse?.first(where: { $0.Currency == selectedCurrency })
        {
            lblAmount.text = clsUtility.formatNumber(rate.Rate)
        }
        
        else
        {
            clsAlertHelper.showConversionRateError(from: self, Title: NSLocalizedString("Error", comment: "Error"), Message: NSLocalizedString("Conversion rates not available", comment: ""))
        }
